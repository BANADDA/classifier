...
// Function to navigate to the details screen
  ...
  Future<String> classifyImage(File imageFile) async {
    ...
    try {
      ...
      // Perform inference
      final output = Float32List(1 * 3);
      _interpreter!.run(input, output);

      // Get predicted class
      final predictedClassIndex =
          output.indexOf(output.reduce((a, b) => a > b ? a : b));
      final predictedClass = _classLabels[predictedClassIndex];
  ...
  void _classifyImage(String option, bool rememberOption) async {
    ...
    if (pickedImagePath != null) {
      ...
      String predictedClass =
          ''; // Initialize predictedClass with a default value
      try {
        predictedClass = await classifyImage(pickedImageFile);
      } catch (e) {
        ...
        builder: (BuildContext context) {
          return CupertinoAlertDialog(
            ...
            actions: [
              ...
              CupertinoDialogAction(
                child: Text('View Details'), // Changed from 'Close'
                onPressed: () {
                  Navigator.of(context).pop(); // Close the dialog
                  _navigateToDetailsScreen(context, imagePath,
                      "Coffee Rust"); // Show the details bottom sheet
                },
              ),
            ],
          );
